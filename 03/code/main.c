...
int calcSquareEq(int a, int b, int c, double* x1, double* x2) {
	// Проверим что пользователь не ввёл что-то вида 0x^2 + ...
	if (a == 0)
		return -1;
	int d;
...
void convertLongShortArray(unsigned int *arr, int len) {
	...
	// arr[i] & 0xFFFF накладывает маску FFFF на младшие два байта в элементе массива и выводит их.
	// arr[i] >> 16 сдвигает число на 16 бит влево 0001 0009 >> 0001
	int i = 0;
